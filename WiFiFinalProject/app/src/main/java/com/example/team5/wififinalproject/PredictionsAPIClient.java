...
public class PredictionsAPIClient {
    ...
    public static File preDirectory = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM+"/FinalProject");
    public static String predictionFlname = "PredictionResults.csv";
    public static File PredictionResults = new File(preDirectory,predictionFlname);
    ...
    private static void predict(Prediction prediction, String text) throws IOException {
        ...
        BufferedWriter bfw = new BufferedWriter(new FileWriter(PredictionResults,true));
        bfw.write(output.getOutputLabel());
        bfw.close();
...
//      System.out.println("Success! Now add code here.");
    ...
    public static void predict() {
        try {
            List<String> testlist = getTestData();
            int length = testlist.size();
            for (int i=0; i<length;i++) {
                predict(client, testlist.get(i));
            }
